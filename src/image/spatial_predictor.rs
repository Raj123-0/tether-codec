...
pub fn select_best_filter(curr: &[u8], prev: &[u8]) -> FilterType {
    ...
    for &f in &candidates {
        ...
        let score: u32 = temp.iter().map(|&r| {
            // Smaller absolute magnitude around 0
...
pub fn compress_image_grayscale(
    ...
    // Header: b"TTHI", width (u32), height (u32), channels (u8 = 1)
    ...
    for y in 0..height as usize {
        ...
        row_residuals.clear();
        filter_row(curr_row, &prev_row, filter, &mut row_residuals);

        // Encode row with rANS
        let mut rans = RansEncoder::new();
        rans.encode_block(&row_residuals, &table);
        let payload = rans.finish();

        for &r in &row_residuals {
            table.observe(r);
        }

        // Row framing: [filter: u8] [len: u32] [payload]
...
pub fn decompress_image_grayscale(
    ...
    if channels != 1 {
        return Err("Only 1-channel grayscale currently supported in streaming decompressor");
    }
    ...
    for _ in 0..h {
        ...
        unfilter_row(&residuals, &prev_row, filter, &mut curr_row);
